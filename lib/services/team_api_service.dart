...
//       if (response.statusCode >= 200 && response.statusCode < 300) {
//         try {
//           // بعد الاستجابة مباشرة
//           print('📥 Raw response from backend: ${response.body}');
...
class TeamApiService {
  ...
  // إنشاء فريق جديد - مع البحث عن ID بعد الإنشاء
  Future<TeamModel?> createTeam({
    required String name,
    required String? projectName,
    required String? description,
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        // ✅ انتظار قليل للتأكد من حفظ البيانات في الباك اند
        await Future.delayed(const Duration(milliseconds: 800));
        
        // ✅ جلب جميع فرق المشرف مرة أخرى
        print('📤 Fetching all teams to get the new team ID...');
        final allTeams = await getMyTeams(supervisorId);
        
        // ✅ البحث عن الفريق الذي تم إنشاؤه بالاسم
        final createdTeam = allTeams.firstWhere(
          (team) => team.name == name,
          orElse: () {
            print('⚠️ Team created but not found by name!');
            // محاولة البحث بالـ projectName
            if (projectName != null && projectName.isNotEmpty) {
              return allTeams.firstWhere(
                (team) => team.projectName == projectName,
                orElse: () {
                  print('⚠️ Team not found by projectName either!');
                  return TeamModel(
                    id: DateTime.now().millisecondsSinceEpoch.toString(),
                    name: name,
                    projectName: projectName,
        ...
        print('✅ Team found with ID: ${createdTeam.id}');
        return createdTeam;
      } else {
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        try {
          ...
          print('📥 Received ${teamsData.length} teams from backend');
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        try {
          ...
          final membersData = data['members'] as List? ?? [];
          for (var m in membersData) {
            // ✅ استبعاد المشرف من قائمة الأعضاء
            if (m['id']?.toString() == data['supervisorId']?.toString()) {
              continue;
            }
            members.add(
              TeamMember(
