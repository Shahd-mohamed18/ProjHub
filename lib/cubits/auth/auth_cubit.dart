...
class AuthCubit extends Cubit<AuthState> {
        ...
        try {
          if (userModel.photoUrl != null && userModel.photoUrl!.isNotEmpty) {
            ...
            if (result['success'] == true) {
              ...
              print('Sync skipped: ${result['message'] ?? result['error']}');
            }
          } else {
            print('No profile image, skipping sync');
          }
        } catch (e) {
          print('Error in sync (ignored): $e');
        }
        _isSyncing = false;
      }
    });
      ...
      if (photoUrl != null && photoUrl.isNotEmpty) {
        if (photoUrl.startsWith('/data/') ||
          ...
          if (await file.exists()) {
            ...
            print('Image file does not exist: $photoUrl');
          }
        } else if (photoUrl != userModel.photoUrl) {
          updateData['photoUrl'] = photoUrl;
        }
  ...
  Future<void> resetPassword({
    ...
    try {
      ...
      _showSnackBar(
        context,
        'Password reset link sent! Check your Spam in Email',
      );
      if (context.mounted) {
        Navigator.pop(context);
      }
