...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    if (user != null) {
      await context.read<ProjectCubit>().loadUserProjects(user.uid);
    }

    if (userModel != null) {
      context.read<TeamsCubit>().loadTeamsForUser(
        userModel.uid,
        userModel.role,
      );
    }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AuthCubit, AuthState>(
      builder: (context, authState) {
        ...
        final userModel = authState.userModel!;
        final isSupervisorOrAssistant =
            userModel.role == UserRole.supervisor ||
  ...
  Widget _buildTeamsSection(UserModel userModel) {
    return BlocBuilder<TeamsCubit, TeamsState>(
      builder: (context, state) {
        ...
        final displayTeams = _showAllProjects ? teams : teams.take(2).toList();
          ...
          child: Column(
            ...
            children: [
              ...
              if (state is TeamsLoading)
                ...
                ...displayTeams.map((team) => _buildTeamCard(team, userModel)),
            ],
          ),
        );
      },
      ...
      child: GestureDetector(
        onTap: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  TeamDetailsScreen(team: team, userRole: userModel.role),
            ),
          );
        },
        child: Column(
