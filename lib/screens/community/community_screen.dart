...
//   @override
//   Widget build(BuildContext context) {
//     final ICommunityRepository repo = useRealApi
...
//     Navigator.push(
//       context,
//       MaterialPageRoute(builder: (_) => CreatePostScreen(cubit: cubit)),
//     );
//   }
...
//       height: 51,
//       color: Colors.white,
//       child: Row(children: [_buildTab('Discover', 0), _buildTab('Profile', 1)]),
//     );
//   }
...
//         if (state is CommunityLoaded) {
// <<<<<<< Updated upstream
//           // ✅ FIX: "My Team" posts are now matched two ways:
//           //   1. post.teamId is in the current user's myTeamIds (preferred —
//           //      relies on the backend's TeamId, now sent on createPost).
//           //   2. Fallback: post author is a known teammate (teamMemberIds),
//           //      kept for posts created before teamId tracking existed, or
//           //      if the backend ever fails to echo TeamId back.
...
//               final matchesTeamId = p.teamId != null &&
//                   p.teamId!.isNotEmpty &&
//                   state.myTeamIds.contains(p.teamId);
//               final matchesTeammate = state.teamMemberIds.contains(p.userId);
//               return matchesTeamId || matchesTeammate;
//             }
// =======
//           final visiblePosts = state.posts.where((p) {
//             if (p.visibility == PostVisibility.public) return true;
...
//             if (p.visibility == PostVisibility.myTeam &&
//                 state.teamMemberIds.contains(p.userId))
//               return true;
// >>>>>>> Stashed changes
//             return false;
//           }).toList();
