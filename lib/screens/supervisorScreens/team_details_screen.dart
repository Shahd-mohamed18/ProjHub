...
class _TeamDetailsScreenState extends State<TeamDetailsScreen> {
  ...
  Future<void> _fetchTeamDetails() async {
    ...
    try {
      final details = await TeamApiService().getTeamDetails(_team.id);
    ...
    return BlocListener<TeamsCubit, TeamsState>(
      ...
      listener: (context, state) {
        if (state is TeamsLoaded) {
          final still = state.teams.any((t) => t.id == _team.id);
          if (!still && mounted) Navigator.pop(context);
        }
      },
      child: _buildScaffold(isSupervisor, canAddTask),
    );
  }

  Widget _buildScaffold(bool isSupervisor, bool canAddTask) {
    if (_isLoading) {
      return Scaffold(
        body: Container(
      ...
      child: Column(
        children: [
          if (_team.assistants.isNotEmpty)
            ..._team.assistants.map((a) => _buildAssistantTile(a))
          else
            Padding(
  ...
  Widget _buildAssistantTile(TeamMember a) {
    return Container(
      ...
      child: Row(
        children: [
          _buildAvatar(a, isAssistant: true),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildStudentTile(TeamMember s) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(12),
      decoration: const BoxDecoration(
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => ChatScreen(
          ...
          otherUserPhoto: m.photoUrl,
        ),
      ),
    );
  }

  Widget _buildAvatar(TeamMember m, {bool isAssistant = false}) {
    ...
    return Stack(
      children: [
        Container(
          ...
          child: _avatarContent(m, url, isAssistant),
        ),
        Positioned(
          right: 0,
          bottom: 0,
