...
class _AllTasksScreenState extends State<AllTasksScreen> {
  ...
  void initState() {
    ...
    _cubit = SupervisorTaskCubit(ApiTaskRepository(), teamsCubit);
  ...
  Widget _buildTeamFilter(
      List<TeamModel> teams, String? selectedTeamId) {
    return Container(
      width: double.infinity,
      height: 60,
        ...
        child: Row(
          children: [
            _buildFilterChip(
              ...
              onTap: () => _cubit.filterByTeam(null),
            ),
            ...
            ...teams.map((team) => Padding(
                  ...
                  child: _buildFilterChip(
                    ...
                    onTap: () => _cubit.filterByTeam(team.id),
                  ),
                )),
          ],
        ),
  ...
  Widget _buildInProgressSection(
    ...
    return Column(
      ...
      children: [
        ...
        ...tasks.map(
            (task) => _buildInProgressCard(context, task, state)),
      ],
    );
  }

  Widget _buildInProgressCard(BuildContext context, TaskModel task,
      SupervisorTasksLoaded state) {
    final teamList =
        state.teams.where((t) => t.id == task.teamId);
    final teamName =
  ...
  Widget _buildCompletedSection(
    ...
    return Column(
      ...
      children: [
        ...
        ...tasks.map(
            (task) => _buildCompletedCard(context, task, state)),
      ],
    );
  }

  Widget _buildCompletedCard(BuildContext context, TaskModel task,
      SupervisorTasksLoaded state) {
    final teamList =
        state.teams.where((t) => t.id == task.teamId);
    final teamName =
